...
static NSString * const VSP_ROUTER_WILDCARD_CHARACTER = @"~";
static NSString *specialCharacters = @"/?&.";

NSString *const VSPRouterParameterURL = @"VSPRouterParameterURL";
NSString *const VSPRouterParameterCompletion = @"VSPRouterParameterCompletion";
NSString *const VSPRouterParameterUserInfo = @"VSPRouterParameterUserInfo";
...
@implementation VSPRouter

+ (instancetype)sharedInstance
{
    static VSPRouter *instance = nil;
...
+ (void)registerURLPattern:(NSString *)URLPattern toHandler:(VSPRouterHandler)handler
{
...
+ (void)openURL:(NSString *)URL withUserInfo:(NSDictionary *)userInfo completion:(void (^)(id result))completion
{
    ...
    if (parameters) {
        VSPRouterHandler handler = parameters[@"block"];
        if (completion) {
            parameters[VSPRouterParameterCompletion] = completion;
        }
        if (userInfo) {
            parameters[VSPRouterParameterUserInfo] = userInfo;
        }
        if (handler) {
...
{
    VSPRouter *router = [VSPRouter sharedInstance];
    
    URL = [URL stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    NSMutableDictionary *parameters = [router extractParametersFromURL:URL matchExactly:NO];
    VSPRouterObjectHandler handler = parameters[@"block"];
...
+ (void)registerURLPattern:(NSString *)URLPattern toObjectHandler:(VSPRouterObjectHandler)handler
{
    [[self sharedInstance] addURLPattern:URLPattern andObjectHandler:handler];
}

- (void)addURLPattern:(NSString *)URLPattern andHandler:(VSPRouterHandler)handler
{
...
- (void)addURLPattern:(NSString *)URLPattern andObjectHandler:(VSPRouterObjectHandler)handler
{
    NSMutableDictionary *subRoutes = [self addURLPattern:URLPattern];
    if (handler && subRoutes) {
        subRoutes[@"_"] = [handler copy];
...
- (NSMutableDictionary *)extractParametersFromURL:(NSString *)url matchExactly:(BOOL)exactly
{
    ...
    parameters[VSPRouterParameterURL] = url;
    ...
    for (NSString* pathComponent in pathComponents) {
        ...
        for (NSString* key in subRoutesKeys) {
            if ([key isEqualToString:pathComponent] || [key isEqualToString:VSP_ROUTER_WILDCARD_CHARACTER]) {
                found = YES;
...
- (NSArray*)pathComponentsFromURL:(NSString*)URL
{
    ...
    if ([URL rangeOfString:@"://"].location != NSNotFound) {
        ...
        if (!URL.length) {
            [pathComponents addObject:VSP_ROUTER_WILDCARD_CHARACTER];
        }
    }
